...
ledgerRouter.get("/ledger/entries", async (_req, res) => {
  try {
    const entries = await db.execute(sql`
      SELECT e.id, e.date::text, e.payment_status, e.created_at,
             json_agg(r ORDER BY r.id) as rows
      FROM ledger_entries e
      LEFT JOIN ledger_rows r ON r.entry_id = e.id
      GROUP BY e.id, e.date, e.payment_status, e.created_at
...
ledgerRouter.post("/ledger/entries", async (req, res) => {
  const { date, payment_status = "unpaid", rows = [] } = req.body;
  if (!date) { res.status(400).json({ error: "date required" }); return; }
  try {
    const [entry] = (await db.execute(sql`
      INSERT INTO ledger_entries (date, payment_status) VALUES (${date}, ${payment_status}) RETURNING *
    `)).rows as any[];
    for (const r of rows) {
...
ledgerRouter.put("/ledger/entries/:id", async (req, res) => {
  ...
  const { date, payment_status, rows } = req.body;
  try {
    if (date || payment_status) {
      const parts: string[] = [];
      if (date) parts.push(`date = '${date}'`);
      if (payment_status) parts.push(`payment_status = '${payment_status}'`);
      await db.execute(sql.raw(`UPDATE ledger_entries SET ${parts.join(", ")} WHERE id = ${id}`));
    }
    if (rows) {
